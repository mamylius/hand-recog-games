...
class HandDetector:
    ...
    def findHands(self, img, draw=True, flipType=True, onlyRight=False, playerid=None):
        ...
        if self.results.multi_hand_landmarks:
            ...
            for handId, (handType, handLms) in enumerate(zip(self.results.multi_handedness, self.results.multi_hand_landmarks)):
                if onlyRight and handType.classification[0].label == "Right":
                    continue
                myHand = {}
                ## lmList
                mylmList = []
    ...
    def thumbAngle(self, hand_landmarks):
        """
        Returns angle (in degrees) of the thumb from a perfect thumbs-up direction.
        Assumes upright, vertical hand with palm facing camera.
        """
        myHandType = hand_landmarks["type"]
        myLmList = hand_landmarks["lmList"]

        # Get the thumb base and tip landmarks
        base = myLmList[2]
        tip = myLmList[4]

        # Compute the thumb vector (from base to tip)
        thumb_vec = [tip[0] - base[0], tip[1] - base[1]]
        thumb_dir = -1 if (tip[0] - base[0]) > 0 else 1
        
        # Reference vector pointing up (in image coordinates: y decreases upwards)
        up_vec = [0, -1]

        # Dot product and magnitude
        dot = thumb_vec[0]*up_vec[0] + thumb_vec[1]*up_vec[1]
        mag_thumb = math.hypot(thumb_vec[0], thumb_vec[1])
        mag_up = 1.0  # magnitude of (0, -1)

        if mag_thumb == 0:
            return 0.0  # Avoid division by zero

        # Calculate angle in radians and then convert to degrees
        angle_rad = math.acos(dot / mag_thumb)
        angle_deg = math.degrees(angle_rad)

        return angle_deg*thumb_dir  # Return angle with sign based on direction
