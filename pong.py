# Multi‑Side Pong – sticky‑collision fix + Winner Podium Screen
...
SCREEN_SIZE = 1000
FPS          = 60
ARENA_RADIUS = 350          # px – distance from centre to wall
BALL_SPEED   = 2.0
BALL_RADIUS  = 10
...
PLAYER_COLORS=[(255,64,64),(64,255,64),(64,128,255),(255,255,64),(255,64,255),(64,255,255)]
...
class Ball:
    ...
    def reset(self):
        ...
        self.vel=pygame.math.Vector2(math.cos(ang),math.sin(ang))*BALL_SPEED
    def update(self): self.pos+=self.vel
    def draw(self,surf): pygame.draw.circle(surf,(255,255,255),(int(self.pos.x),int(self.pos.y)),BALL_RADIUS)

# ------------------------- GAME FUNCTIONS --------------------------

def draw_score(surf,scores,pads):
    f=pygame.font.SysFont(None,32); x=20
    for i,s in enumerate(scores):
        t=f.render(f"P{i+1}:{s}",True,pads[i].color); surf.blit(t,(x,10)); x+=t.get_width()+20

def resolve_paddle_hit(ball,rel,paddle):
    """Reflect ball, add spin, and push it just inside arena to avoid stutter."""
    norm=rel.normalize(); tan=pygame.math.Vector2(-norm.y,norm.x)
    # reflect
    ball.vel-=2*ball.vel.dot(norm)*norm
    # subtle spin
    ball.vel+=tan*(paddle.doff*ARENA_RADIUS*SPIN_FACTOR)
    # keep moderate speed
    speed=max(BALL_SPEED*0.8,min(ball.vel.length(),BALL_SPEED*2))
    ball.vel.scale_to_length(speed)
...
def show_podium(screen,scores,pads):
    ...
    # Pre‑compute podium geometry
    cx=SCREEN_SIZE//2
    centres=[cx+(i-1)*(PODIUM_WIDTH+PODIUM_GAP) for i in range(topN)]  # i = 0,1,2 → left, mid, right
    ...
    while True:
        ...
        for pos in range(topN):
            ...
            # Dark top edge for visual depth
...
def run_game(n):
    ...
    scores=[0]*n; clock=pygame.time.Clock(); font=pygame.font.SysFont(None,48)
    rounds=0
    while rounds<NUM_ROUNDS:
        # ----------------- Round initialisation --------------------
        for p in pads:p.reset()
        ball=Ball(); waiting=True
        # --- waiting for space to serve
        while waiting:
            for e in pygame.event.get():
                if e.type==pygame.QUIT: pygame.quit(); sys.exit()
                if e.type==pygame.KEYDOWN and e.key==pygame.K_SPACE: waiting=False
            screen.fill((0,0,0))
            screen.blit(font.render(f"Round {rounds+1}/{NUM_ROUNDS} – SPACE",True,(200,200,200)),(SCREEN_SIZE//2-200,SCREEN_SIZE//2-24))
            draw_score(screen,scores,pads); pygame.display.flip(); clock.tick(FPS)
        ...
        while playing:
            ...
            for e in pygame.event.get():
                if e.type==pygame.QUIT: pygame.quit(); sys.exit()
            pr=pygame.key.get_pressed(); [p.update(pr) for p in pads]
            ball.update()
            rel=pygame.math.Vector2(ball.pos.x-SCREEN_SIZE//2,ball.pos.y-SCREEN_SIZE//2)
            if rel.length()>=ARENA_RADIUS-BALL_RADIUS:
                ang=angle_normalise(math.atan2(rel.y,rel.x))
                hit=None
                for p in pads:
                    if angle_between(ang,p.start-COLLISION_MARGIN,p.end+COLLISION_MARGIN): hit=p; break
                if hit:
                    resolve_paddle_hit(ball,rel,hit)
                else:
                    # ---------------- Miss – award points ----------
                    side=2*math.pi/n; loser=int(((ang-1e-6)//side)%n)
                    for i in range(n):
                        if i!=loser: scores[i]+=1
                    rounds+=1
                    # Flash message
                    t0=pygame.time.get_ticks(); miss=font.render(f"P{loser+1} missed",True,PLAYER_COLORS[loser])
                    while pygame.time.get_ticks()-t0<1200:
                        for e in pygame.event.get():
                            if e.type==pygame.QUIT: pygame.quit(); sys.exit()
                        screen.fill((0,0,0)); screen.blit(miss,miss.get_rect(center=(SCREEN_SIZE//2,SCREEN_SIZE//2)))
                        draw_score(screen,scores,pads); pygame.display.flip(); clock.tick(FPS)
                    playing=False
            # draw
            screen.fill((0,0,0)); pygame.draw.circle(screen,(80,80,80),(SCREEN_SIZE//2,SCREEN_SIZE//2),ARENA_RADIUS,1)
            [p.draw(screen) for p in pads]; ball.draw(screen); draw_score(screen,scores,pads); pygame.display.flip()
...
def menu():
    ...
    font=pygame.font.SysFont(None,48); sel=2; clock=pygame.time.Clock()
    while True:
        for e in pygame.event.get():
            if e.type==pygame.QUIT: pygame.quit(); sys.exit()
...
if __name__=="__main__":
    pygame.init(); screen=pygame.display.set_mode((SCREEN_SIZE,SCREEN_SIZE))
    while True:
        players=menu()
        run_game(players)
